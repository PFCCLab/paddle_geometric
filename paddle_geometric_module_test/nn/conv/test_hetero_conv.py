...
def test_hetero_conv(aggr):
    data = HeteroData()
    data['paper'].x = paddle.randn(shape=[50, 32])
...
def test_hetero_conv_with_custom_conv():
    data = HeteroData()
    data['paper'].x = paddle.randn(shape=[50, 32])
    data['paper'].y = paddle.randn(shape=[50, 3])
    data['paper'].z = paddle.randn(shape=[50, 3])
    data['author'].x = paddle.randn(shape=[30, 64])
    data['author'].y = paddle.randn(shape=[30, 3])
    data['author'].z = paddle.randn(shape=[30, 3])
    data['paper', 'paper'].edge_index = get_random_edge_index(50, 50, 200)
    data['paper', 'author'].edge_index = get_random_edge_index(50, 30, 100)
    data['author', 'paper'].edge_index = get_random_edge_index(30, 50, 100)
...
def test_hetero_conv_with_dot_syntax_node_types():
    data = HeteroData()
    data['src.paper'].x = paddle.randn(shape=[50, 32])
    data['author'].x = paddle.randn(shape=[30, 64])
...
def test_compile_hetero_conv_graph_breaks(device):
    ...
    for key in expected.keys():
        assert paddle.allclose(out[key], expected[key], atol=1e-6)
...
if __name__ == '__main__':
    ...
    compiled_model = paddle.compile(model)
