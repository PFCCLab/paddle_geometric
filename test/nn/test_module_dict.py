...
def test_dot_syntax_keys():
    module_dict = ModuleDict({
        'lin1': paddle.nn.Linear(16, 16),
        'model.lin2': paddle.nn.Linear(8, 8),
        'model.sub_model.lin3': paddle.nn.Linear(4, 4),
    })
...
def test_tuple_keys():
    module_dict = ModuleDict({
        ('a', 'b'): paddle.nn.Linear(16, 16),
        ('a.b', 'c'): paddle.nn.Linear(8, 8),
    })
...
def test_reserved_keys():
    module_dict = ModuleDict({
        'type': paddle.nn.Linear(16, 16),
        '__annotations__': paddle.nn.Linear(8, 8),
    })
