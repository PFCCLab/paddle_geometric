...
def is_negative(edge_index, neg_edge_index, size, bipartite):
    ...
    neg_adj[neg_edge_index[0].cast(paddle.int64),
            neg_edge_index[1].cast(paddle.int64)] = True
...
def test_batched_negative_sampling():
    edge_index = paddle.to_tensor([[0, 0, 1, 2], [0, 1, 2, 3]])
    edge_index = paddle.cat([edge_index, edge_index + 4], dim=1)
...
def test_bipartite_batched_negative_sampling():
    ...
    edge_index = paddle.cat([edge_index1, edge_index2, edge_index3], dim=1)
    ...
    adj[edge_index[0].cast(paddle.int64),
        edge_index[1].cast(paddle.int64)] = True
...
def test_structured_negative_sampling():
    ...
    edge_index = paddle.LongTensor([[0, 0, 1, 1, 2], [1, 2, 0, 2, 1]])
...
def test_structured_negative_sampling_feasible():
    edge_index = paddle.LongTensor([[0, 0, 1, 1, 2, 2, 2],
                                    [1, 2, 0, 2, 0, 1, 1]])
