...
CALCULATION_VIA_PTR_AVAILABLE = (paddle_geometric.typing.WITH_SOFTMAX or
                                 paddle_geometric.typing.WITH_PADDLE_SCATTER)
...
def test_softmax_backward():
    ...
    src_sparse.requires_grad = True
    src_dense.requires_grad = True
...
def test_softmax_dim():
    ...
    src = paddle.randn(4, 16)
    assert paddle.allclose(softmax(src, index, dim=0), src.softmax(dim=0))
    assert paddle.allclose(softmax(src, ptr=ptr, dim=0), src.softmax(dim=0))

    src = paddle.randn(4, 4)
    ...
    src = paddle.randn(4, 4, 16)
    assert paddle.allclose(softmax(src, index, dim=1), src.softmax(dim=1))
    if CALCULATION_VIA_PTR_AVAILABLE:
        assert paddle.allclose(softmax(src, ptr=ptr, dim=1),
                               src.softmax(dim=1))
    else:
        with pytest.raises(ImportError, match="requires the 'paddle-scatter'"):
            softmax(src, ptr=ptr, dim=1)
...
#     x = paddle.randn(num_edges, 64, device=args.device)
#     index = paddle.randint(num_nodes, (num_edges, ), device=args.device)

#     compiled_softmax = paddle.compile(softmax)
