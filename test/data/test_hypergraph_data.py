...
def test_hypergraph_data():
    ...
    assert paddle.equal_all(data.__inc__('edge_index', data.edge_index),
                       paddle.to_tensor([[data.num_nodes], [data.num_edges]])).item()
    data_list = [data, data]
    ...
    data.edge_attr = paddle.randn(data.num_edges, 2)
...
def test_hypergraphdata_subgraph():
    ...
    assert len(out) == 5
    assert paddle.equal_all(out.x, paddle.to_tensor([1, 2, 4])).item()
    assert paddle.equal_all(out.y, data.y).item()
    assert out.edge_index.tolist() == [[1, 2, 2, 1, 0, 1], [0, 0, 1, 1, 2, 2]]
    assert paddle.equal_all(out.edge_attr, edge_attr[[1, 2, 3]]).item()
    ...
    assert paddle.equal_all(out.edge_attr, edge_attr[[1, 2]]).item()
