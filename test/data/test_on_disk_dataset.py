...
def test_pickle(tmp_path):
    ...
    out = dataset.get(0)
    assert paddle.equal_all(out.x, data_list[0].x).item()
    assert paddle.equal_all(out.edge_index, data_list[0].edge_index).item()
    ...
    for out, data in zip(out_list, data_list[1:]):
        ...
        assert paddle.equal_all(out.edge_index, data.edge_index).item()
