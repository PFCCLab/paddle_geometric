...
def get_random_edge_index(num_src, num_dst, num_edges):
    """
    Generate random edge index tensor for testing.
    ...
    edge_index = paddle.stack([
        paddle.randint(0, num_src, (num_edges,))
        for _ in range(2)
    ])
    # Set second dimension to destination range
    edge_index[1] = paddle.randint(0, num_dst, (num_edges,))
    return edge_index
...
def test_batch_basic():
    ...
    # Test multi-data batch with follow_batch
    batch = Batch.from_data_list([data1, data2, data3], follow_batch=['string'])
...
def test_index():
    """Test Index type preservation during batching."""
    ...
    # Test unbatching preserves Index attributes
...
def test_edge_index():
    """Test EdgeIndex type preservation during batching."""
    ...
    # Test unbatching preserves EdgeIndex attributes
...
def test_batch_with_paddle_coo_tensor():
    ...
    assert batch.num_graphs == len(batch) == 3
    assert batch.x.to_dense().reshape([-1]).tolist() == [1, 2, 3, 1, 2, 1, 2, 3, 4]
    assert batch.batch.tolist() == [0, 0, 0, 1, 1, 2, 2, 2, 2]
    assert batch.ptr.tolist() == [0, 3, 5, 9]
...
def test_batching_with_new_dimension():
    """Test batching with custom __cat_dim__ returning None."""
    paddle_geometric.set_debug(True)
...
def test_recursive_batch():
    """Test recursive batching with dict and list attributes."""
    data1 = Data(
        x={
            '1': paddle.randn(10, 32),
            '2': paddle.randn(20, 48)
        },
        edge_index=[
    ...
    data2 = Data(
        x={
            '1': paddle.randn(20, 32),
            '2': paddle.randn(40, 48)
        },
        edge_index=[
    ...
    assert paddle.allclose(batch.x['1'],
                          paddle.concat([data1.x['1'], data2.x['1']], axis=0)).item()
    assert paddle.allclose(batch.x['2'],
                          paddle.concat([data1.x['2'], data2.x['2']], axis=0)).item()
...
def test_batching_of_batches():
    """Test batching of already batched data."""
    data = Data(x=paddle.randn(2, 16))
...
def test_hetero_batch():
    ...
    e2 = ('a', 'p')
    data1 = HeteroData()
    data1['p'].x = paddle.randn(100, 128)
    data1['a'].x = paddle.randn(200, 128)
    data1[e1].edge_index = get_random_edge_index(100, 200, 500)
    data1[e1].edge_attr = paddle.randn(500, 32)
    data1[e2].edge_index = get_random_edge_index(200, 100, 400)
    data1[e2].edge_attr = paddle.randn(400, 32)

    data2 = HeteroData()
    data2['p'].x = paddle.randn(50, 128)
    data2['a'].x = paddle.randn(100, 128)
    data2[e1].edge_index = get_random_edge_index(50, 100, 300)
    data2[e1].edge_attr = paddle.randn(300, 32)
    data2[e2].edge_index = get_random_edge_index(100, 50, 200)
    data2[e2].edge_attr = paddle.randn(200, 32)
    ...
    assert paddle.allclose(
        batch[e1].edge_attr,
        paddle.concat([data1[e1].edge_attr, data2[e1].edge_attr], axis=0))
    ...
    assert paddle.allclose(
        batch[e2].edge_attr,
        paddle.concat([data1[e2].edge_attr, data2[e2].edge_attr], axis=0))
...
def test_pair_data_batching():
    ...
    x_s = paddle.randn(5, 16)
    edge_index_s = paddle.to_tensor([
        [0, 0, 0, 0],
        [1, 2, 3, 4],
    ])
    x_t = paddle.randn(4, 16)
    ...
    assert paddle.allclose(batch.x_s, paddle.concat([x_s, x_s], axis=0))
    assert batch.edge_index_s.tolist() == [[0, 0, 0, 0, 5, 5, 5, 5],
                                           [1, 2, 3, 4, 6, 7, 8, 9]]

    assert paddle.allclose(batch.x_t, paddle.concat([x_t, x_t], axis=0))
    assert batch.edge_index_t.tolist() == [[0, 0, 0, 4, 4, 4],
                                           [1, 2, 3, 5, 6, 7]]


def test_batch_with_empty_list():
    """Test batching with empty list attributes."""
    x = paddle.randn(4, 1)
    edge_index = paddle.to_tensor([[0, 1, 1, 2, 2, 3], [1, 0, 2, 1, 3, 2]])
...
def test_nested_follow_batch():
    """Test follow_batch with nested structures."""
    def tr(n, m):
...
def test_batch_with_sparse_tensor():
    """Test batching with sparse tensors."""
...
def test_batch_with_sparse_tensor_coo():
    ...
    x1 = paddle.to_tensor([[1.0], [2.0], [3.0]]).to_sparse_coo()
    edge_index1 = paddle.to_tensor([[0, 1, 1, 2], [1, 0, 2, 1]])
    adj1 = SparseTensor.from_edge_index(edge_index1)
    data1 = Data(x=x1, adj=adj1)

    x2 = paddle.to_tensor([[1.0], [2.0]]).to_sparse_coo()
    edge_index2 = paddle.to_tensor([[0, 1], [1, 0]])
    adj2 = SparseTensor.from_edge_index(edge_index2)
    data2 = Data(x=x2, adj=adj2)
    ...
    assert batch.num_graphs == len(batch) == 2
    assert batch.x.shape[0] == 5  # 3 + 2

    # Verify sparse format is preserved
    assert batch.x.is_sparse()

    # Verify edge indices are correctly concatenated
    assert batch.adj.coo()[0].tolist() == [0, 1, 1, 2, 3, 4]
    assert batch.adj.coo()[1].tolist() == [1, 0, 2, 1, 3, 4]

    # Test unbatching
    data_list = batch.to_data_list()
    assert len(data_list) == 2
    assert data_list[0].x.to_dense().tolist() == [[1.0], [2.0], [3.0]]
    assert data_list[1].x.to_dense().tolist() == [[1.0], [2.0]]
