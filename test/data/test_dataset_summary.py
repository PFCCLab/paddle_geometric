...
from paddle_geometric.data.summary import Stats, Summary
...
def check_stats(stats: Stats, expected: Tensor):
    expected = expected.to(paddle.float32)
...
def test_dataset_summary():
    ...
    num_nodes = paddle.to_tensor([data.num_nodes for data in dataset])
    num_edges = paddle.to_tensor([data.num_edges for data in dataset])
...
def test_dataset_summary_hetero_per_type_check():
    dataset = FakeHeteroDataset(num_graphs=10)
    exp_num_nodes = paddle.to_tensor([data.num_nodes for data in dataset])
    exp_num_edges = paddle.to_tensor([data.num_edges for data in dataset])
    ...
    for node_type in dataset.node_types:
        ...
        num_nodes_per_type[node_type] = paddle.to_tensor(num_nodes)
    ...
    for edge_type in dataset.edge_types:
        ...
        num_edges_per_type[edge_type] = paddle.to_tensor(num_edges)
