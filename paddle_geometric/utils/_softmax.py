...
def softmax(
    src: paddle.Tensor,
    index: Optional[paddle.Tensor] = None,
    ptr: Optional[paddle.Tensor] = None,
    ...
    if (ptr is not None and src.place.is_cpu_place()
            and paddle_geometric.typing.WITH_SOFTMAX and not is_compiling()):
    ...
    if ptr is not None and (ptr.dim() == 1 or ptr.dim() > 1 and index is None
        ...
        src_max = src_max.repeat_interleave(repeats=count, axis=dim)
        out = (src - src_max).exp()
        out_sum = segment(out, ptr, reduce="sum") + 1e-16
        out_sum = out_sum.repeat_interleave(repeats=count, axis=dim)
    elif index is not None:
        N = maybe_num_nodes(index, num_nodes)
        src_max = scatter(src.detach(), index, dim, dim_size=N, reduce="max")
        out = src - src_max.index_select(axis=dim, index=index)
        out = out.exp()
        out_sum = scatter(out, index, dim, dim_size=N, reduce="sum") + 1e-16
        out_sum = out_sum.index_select(axis=dim, index=index)
    else:
