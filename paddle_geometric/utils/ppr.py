...
def get_ppr(
    ...
    col = paddle.tensor(list(chain.from_iterable(cols)), device=device)
    weight = paddle.tensor(list(chain.from_iterable(weights)), device=device)
    deg = paddle.tensor([len(value) for value in cols], device=device)
    row = paddle.arange(end=num_nodes) if target is None else target
    row = row.repeat_interleave(repeats=deg)
