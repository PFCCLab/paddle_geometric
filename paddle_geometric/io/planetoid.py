...
def read_planetoid_data(folder: str, prefix: str) -> Data:
    ...
    train_index = paddle.arange(dtype="int64", end=y.shape[0])
    val_index = paddle.arange(start=y.shape[0], end=y.shape[0] + 500, dtype="int64")
    sorted_test_index = (paddle.sort(x=test_index), paddle.argsort(x=test_index))[0]
    if prefix.lower() == "citeseer":
        len_test_indices = int(test_index._max() - test_index._min()) + 1
        tx_ext = paddle.zeros(shape=[len_test_indices, tx.shape[1]], dtype=tx.dtype)
        tx_ext[sorted_test_index - test_index._min(), :] = tx
        ty_ext = paddle.zeros(shape=[len_test_indices, ty.shape[1]], dtype=ty.dtype)
        ty_ext[sorted_test_index - test_index._min(), :] = ty
        tx, ty = tx_ext, ty_ext
    if prefix.lower() == "nell.0.001":
        tx_ext = paddle.zeros(shape=[len(graph) - allx.shape[0], x.shape[1]])
        tx_ext[sorted_test_index - allx.shape[0]] = tx
        ty_ext = paddle.zeros(shape=[len(graph) - ally.shape[0], y.shape[1]])
        ty_ext[sorted_test_index - ally.shape[0]] = ty
        tx, ty = tx_ext, ty_ext
        x = paddle.concat(x=[allx, tx], axis=0)
        ...
        row = paddle.concat(x=[row, isolated_idx])
        col = paddle.concat(
            x=[col, paddle.arange(end=isolated_idx.shape[0]) + x.shape[1]]
        )
        value = paddle.concat(
        ...
        x[test_index] = x[sorted_test_index]
    y = (
        paddle.concat(x=[ally, ty], axis=0).max(axis=1),
        paddle.concat(x=[ally, ty], axis=0).argmax(axis=1),
    )[1]
    ...
    edge_index = edge_index_from_dict(graph_dict=graph, num_nodes=y.shape[0])
