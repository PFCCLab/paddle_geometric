...
def parse_sdf(src: str) -> Data:
    ...
    x = paddle.to_tensor(data=[elems[item.split()[3]] for item in atom_block])
    x = one_hot(x, num_classes=len(elems))
    bond_block = lines[1 + num_atoms : 1 + num_atoms + num_bonds]
    row, col = parse_txt_array(bond_block, end=2, dtype="int64").t() - 1
    row, col = paddle.concat(x=[row, col], axis=0), paddle.concat(x=[col, row], axis=0)
