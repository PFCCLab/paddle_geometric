...
class ResGatedGraphConv(MessagePassing):
    ...
    def message(self, k_i: Tensor, q_j: Tensor, v_j: Tensor,
        ...
        if edge_attr is not None:
            k_i = self.lin_key(paddle.concat([k_i, edge_attr], axis=-1))
            q_j = self.lin_query(paddle.concat([q_j, edge_attr], axis=-1))
            v_j = self.lin_value(paddle.concat([v_j, edge_attr], axis=-1))
