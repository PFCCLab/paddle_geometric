...
class PointGNNConv(MessagePassing):
    ...
    Args:
        mlp_h (paddle.nn.Layer): A neural network :math:`h_{\mathbf{\Theta}}`
            that maps node features of size :math:`F_{in}` to three-dimensional
            coordination offsets :math:`\Delta \textrm{pos}_i`.
        mlp_f (paddle.nn.Layer): A neural network :math:`f_{\mathbf{\Theta}}`
        ...
        mlp_g (paddle.nn.Layer): A neural network :math:`g_{\mathbf{\Theta}}`
