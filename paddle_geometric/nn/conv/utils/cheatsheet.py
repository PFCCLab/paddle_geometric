...
def _get_conv_class(cls: str):
    return importlib.import_module('paddle_geometric.nn.conv').__dict__[cls]


def paper_title(cls: str) -> Optional[str]:
    cls = _get_conv_class(cls)
...
def supports_bipartite_graphs(cls: str) -> bool:
    ...
    return any(token in signature_str for token in [
        'OptPairTensor',
        'PairTensor',
        'Tuple[Tensor',
        'Tuple[paddle.Tensor',
