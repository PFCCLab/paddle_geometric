...
class HypergraphConv(MessagePassing):
    ...
    .. code-block:: python
        ...
        hyperedge_index = paddle.to_tensor([
            [0, 1, 2, 1, 2, 3],
            [0, 0, 0, 1, 1, 1],
        ])
    ...
    def forward(self, x: Tensor, hyperedge_index: Tensor,
        ...
        if hyperedge_weight is None:
            hyperedge_weight = paddle.ones([num_edges], dtype=x.dtype)
        ...
        if self.use_attention:
            ...
            hyperedge_attr = hyperedge_attr.reshape([-1, self.heads,
                                                     self.out_channels])
            x_i = x[hyperedge_index[0]]
            x_j = hyperedge_attr[hyperedge_index[1]]
            alpha = (paddle.concat([x_i, x_j], axis=-1) * self.att).sum(axis=-1)
        ...
        B = scatter(paddle.ones([hyperedge_index.shape[1]]), hyperedge_index[1],
    ...
    def message(self, x_j: Tensor, norm_i: Tensor, alpha: Tensor) -> Tensor:
        ...
        out = norm_i.unsqueeze(-1).unsqueeze(-1) * x_j.reshape([-1, H, F])
