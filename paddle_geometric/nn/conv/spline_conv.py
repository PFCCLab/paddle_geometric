...
class SplineConv(MessagePassing):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if spline_basis is None:
            raise ImportError("'SplineConv' requires 'paddle-spline-conv'")
        ...
        kernel_size = paddle.to_tensor(repeat(kernel_size, dim), dtype=paddle.int64)
        self.register_buffer('kernel_size', kernel_size)

        is_open_spline = repeat(is_open_spline, dim)
        is_open_spline = paddle.to_tensor(is_open_spline, dtype=paddle.uint8)
        ...
        # Initialize the weight parameter in static graph mode
        if in_channels[0] > 0:
    ...
    def message(self, x_j: Tensor, edge_attr: Tensor) -> Tensor:
        """
        Compute the message for each neighbor.
        """
