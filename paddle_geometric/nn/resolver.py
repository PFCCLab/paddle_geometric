...
import paddle
from paddle.callbacks import ReduceLROnPlateau
...
def activation_resolver(query: Union[Any, str] = 'relu', *args, **kwargs):
    base_cls = paddle.nn.Layer
    base_cls_repr = 'Act'
    acts = [
        getattr(paddle.nn, act) for act in dir(paddle.nn)
        if isinstance(getattr(paddle.nn, act), type)
...
def normalization_resolver(query: Union[Any, str], *args, **kwargs):
    import paddle_geometric.nn.norm as norm
    base_cls = paddle.nn.Layer
    base_cls_repr = 'Norm'
    norms = [
...
def aggregation_resolver(query: Union[Any, str], *args, **kwargs):
    import paddle_geometric.nn.aggr as aggr
...
def optimizer_resolver(query: Union[Any, str], *args, **kwargs):
    base_cls = Optimizer
    optimizers = [
        optimizer for optimizer in vars(paddle.optimizer).values()
...
def lr_scheduler_resolver(
    ...
    **kwargs,
    ...
    PyG or Paddle from its name or type.
    ...
    # classes = [
    #               scheduler for scheduler in vars(paddle.optimizer.lr).values()
