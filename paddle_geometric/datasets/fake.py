...
from paddle_geometric.utils import coalesce, remove_self_loops, to_undirected
...
class FakeDataset(InMemoryDataset):
    ...
    def generate_data(self) -> Data:
        ...
        if self.edge_dim > 1:
            data.edge_attr = paddle.rand([data.num_edges, self.edge_dim])
        elif self.edge_dim == 1:
            data.edge_weight = paddle.rand([data.num_edges])
...
class FakeHeteroDataset(InMemoryDataset):
    ...
    def generate_data(self) -> HeteroData:
        ...
        for i, (node_type, num_channels) in enumerate(iterator):
            ...
            if num_channels > 0:
                store.x = paddle.randn([num_nodes, num_channels])
            else:
                store.num_nodes = num_nodes

            if self._num_classes > 0 and self.task == 'node' and i == 0:
                store.y = paddle.randint(self._num_classes, shape=[num_nodes])
        ...
        for (src, rel, dst) in self.edge_types:
            ...
            if self.edge_dim > 1:
                store.edge_attr = paddle.rand([store.num_edges, self.edge_dim])
            elif self.edge_dim == 1:
